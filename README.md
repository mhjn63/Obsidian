> HTML Page: [[HTML Pages/Welcome.html|Open HTML Page]]

# The Mastermind Notes

### Cybersecurity · Offensive & Defensive · Practitioner-First

> **Stop reading docs. Start thinking like an attacker.**

Field-ready notes, methodology breakdowns, and certification cheat sheets built by a practitioner, for practitioners. 

**→ [Browse Free Notes](https://publish.obsidian.md/cybersecuritycheatsheets/)** | **→ [Unlock Premium](https://buymeacoffee.com/notescatalog/membership)**
## By the Numbers

|||
|---|---|
|📺 **63K+**|YouTube Subscribers|
|🧪 **30K+**|Web Visitors|
|📋 **4K+**|Students and Professionals Using The Notes|

---

## What's in the Vault

Four tiers. One clear mission.

Whether you're just getting started or deep in the trenches, there's a tier built for where you are right now. Free notes cover the essentials , premium unlocks the full playbook.

---

## What You'll Actually Use

Notes built around real engagements, real exam objectives, and real SOC workflows — not a rehash of vendor documentation.

|Topic|Focus|
|---|---|
|`#` **Penetration Testing**|OSCP · CPTS · HTB|
|`#` **Web App Security**|OSWE · Bug Bounty|
|`#` **SOC & Blue Team**|CDSA · SIEM · IR|
|`#` **Threat Intelligence**|TAXII · YARA · MITRE|
|`#` **Malware Analysis**|Reverse Engineering|
|`#` **AI Red Teaming**|Garak · PyRIT · LLM Sec|
|`#` **Network Security**|Active Directory · Pivoting|
|`#` **Tooling & Automation**|Scripts · Integrations|

### Cert Coverage

`OSCP` `CPTS` `OSWE` `HTB CDSA` `CEH` `CompTIA Sec+` `eJPT`

---

## The Author : Motasem Hamdan

I'm a **cybersecurity practitioner, technical writer, and content creator** who got tired of resources that treat readers like beginners forever.

My notes are built the way I wish someone had built them when I was grinding through certs and CTFs — methodology-first, practitioner-grade, and structured for **how analysts actually think** on the job.

Over **62,000 people on YouTube** follow along. Thousands more read on the site every month. These aren't notes for passing an exam and forgetting everything — they're references you'll keep coming back to.

```
❯ whoami
motasem_hamdan — cybersec_practitioner

❯ cat expertise.txt
offensive_security: advanced
blue_team_soc:      advanced
threat_intel:       advanced
technical_writing:  practitioner-grade

❯ cat philosophy.txt
"teach how to think, not just what to type."
```

### Find Me Here

||Link|
|---|---|
|▶ YouTube|[youtube.com/@MotasemHamdan](https://www.youtube.com/@MotasemHamdan) · 62K+ subscribers|
|✦ Main Site|[motasem-notes.net](https://motasem-notes.net/)|
|⊡ Store|[shop.motasem-notes.net](https://shop.motasem-notes.net/)|

---

## Membership

One subscription. Everything unlocked.

Skip the hours lost searching fragmented resources. One membership gives you the full library, updated continuously as the threat landscape evolves.

---

### Free — $0/forever

- ✓ Foundational notes library
- ✓ Selected HTB summaries
- ✓ YouTube companion write-ups
- ✓ Tool overview guides

**[Start Reading →](https://publish.obsidian.md/cybersecuritycheatsheets/)/)**

---

### The Research Vault ⭐
Get access to exclusive cybersecurity writeups, threat analysis posts, malware investigations, and hands-on technical breakdowns not published anywhere else. 

Members receive early access to premium research, practical SOC and threat intelligence insights, reverse engineering notes, and in-depth walkthroughs designed to help security professionals stay ahead of emerging threats and sharpen real-world skills.

**[Become a Member →](https://shop.motasem-notes.net)**

----------------------
### ⭐⭐ Premium Member — $15/month _(Most Popular · Cancel anytime)_

- ✓ Full notes library (100+ resources)
- ✓ Cert prep discounts: OSCP, CPTS, OSWE, CDSA
- ✓ Complete HTB machine writeups
- ✓ SOC analyst methodology playbooks
- ✓ AI Red Teaming notes
- ✓ Threat intelligence guides
- ✓ New content every week
- ✓ Priority access to new releases

**[Become a Member →](https://buymeacoffee.com/notescatalog/membership)**

---

### The Cyber Intelligence Brief ⭐⭐⭐ — $20/month _(For Security leaders, analysts, practitioners)_

A weekly intelligence memo on cybersecurity and technology. 

Each edition distills complex security, tech, and risk developments into clear insights, strategic lessons, and practical takeaways. You’ll understand why incidents happen, what trends actually matter, and how to think about security decisions without being a specialist.

- ✓ Get Strategic cyber security and tech insights weekly to your email every *Saturday*

**[Become a Member →](https://buymeacoffee.com/notescatalog/membership)**

----------------------------------------------------------------------------

### Store  _(One-time purchase)_

- ✓ Buy individual cheat sheets
- ✓ Downloadable PDFs & guides
- ✓ No recurring commitment
- ✓ Yours to keep permanently

**[Browse Store →](https://buymeacoffee.com/notescatalog/extra)**

---

## Reviews
![[2.png]]
![[3 .png]]
![[4.png]]
![[5.png]]
![[6.png]]
![[7.png]]
![[8.png]]
![[9.png]]
## FAQ

<details> <summary><strong>Are the notes beginner-friendly?</strong></summary>

The free tier has solid foundational content. Premium notes are written for intermediate-to-advanced practitioners — they assume you know the basics and want to go deeper. If you're grinding toward OSCP or working in a SOC, you'll feel right at home.

</details> <details> <summary><strong>How often is new content added?</strong></summary>

Continuously. New walkthroughs, methodology updates, and cheat sheets drop regularly — aligned with new HTB machines, cert updates, and emerging threat topics. As a member, you get access to everything as it lands.

</details> <details> <summary><strong>Can I cancel anytime?</strong></summary>

Yes, absolutely. Membership is managed through Buy Me a Coffee — you can cancel any time directly from your account. No long-term lock-in, no awkward cancellation flows.

</details> <details> <summary><strong>What's the difference between the membership and the store?</strong></summary>

The [membership](https://buymeacoffee.com/notescatalog/membership) gives you ongoing access to the library for a monthly fee. The [store](https://shop.motasem-notes.net/) lets you buy individual resources once and own them permanently — good if you just need one specific cert pack.

</details> <details> <summary><strong>Is there a YouTube channel I can check first?</strong></summary>

Definitely. Head to [@MotasemHamdan](https://www.youtube.com/@MotasemHamdan) on YouTube — over 62K subscribers and a large back-catalogue of walkthroughs, tool demos, and methodology breakdowns. Best way to see if the teaching style clicks for you before committing to anything.

</details>

---

_Practitioner-grade cybersecurity notes & resources._

[Site](https://motasem-notes.net/) · [Store](https://shop.motasem-notes.net/) · [YouTube](https://www.youtube.com/@MotasemHamdan) · [Membership](https://buymeacoffee.com/notescatalog/membership)